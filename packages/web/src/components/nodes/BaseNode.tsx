...
/**
 * Minimal fallback node renderer — used only in contexts where a full
 * node component is not yet available.  Honors the full NodeProps<Node<FlowNodeData>>
 * contract so it is substitutable wherever XYFlow expects a node component (LSP).
 */
export function BaseNode({ data, type, selected }: NodeProps<Node<FlowNodeData, string>>) {
  const inputs = data.inputsSchema || [];
  const outputs = data.outputsSchema || [];

  return (
    <div
      className={`border bg-card text-card-foreground shadow-sm w-64 ${
        selected ? 'border-primary ring-1 ring-primary' : 'border-border'
      }`}
    >
      <div className="flex items-center gap-2 border-b border-border bg-muted/50 p-3">
        {icons[type] ?? <Code size={16} />}
