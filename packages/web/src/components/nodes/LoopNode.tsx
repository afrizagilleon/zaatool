  ...
  if (isVertical) {
    return (
      ...
      >
        ...
        {/* Middle: Header */}
        <div className="flex items-center gap-2 px-3 py-2 border-y border-node-loop/20 bg-node-loop/5">
          ...
          <span className="text-[9px] font-semibold uppercase tracking-wide px-1.5 py-0.5 bg-node-loop/10 text-node-loop">LOOP</span>
          <StatusDot nodeId={id} />
        </div>
        ...
        {outputs.map((output, idx) => {
          ...
          let handleColorClass = '!bg-handle';

          if (output.name === 'element') {
            handleColorClass = '!bg-node-loop';
          } else if (output.name === 'body') {
            handleColorClass = '!bg-node-loop/70';
          } else if (output.name === 'completed') {
            handleColorClass = '!bg-status-done';
          }
        ...
        {outputs.length > 0 && (
          <div className="flex justify-center gap-6 px-3 pt-1 pb-3">
            {outputs.map((output) => {
              ...
              if (output.name === 'element') {
                textColorClass = 'text-node-loop';
              } else if (output.name === 'body') {
                textColorClass = 'text-node-loop/70';
              } else if (output.name === 'completed') {
                textColorClass = 'text-status-done';
              }
              ...
              return (
                <div key={`out-${output.name}`} className="flex flex-col items-center gap-0.5 mb-1">
                  <span className={cn('text-[9px] font-medium transition-colors', textColorClass)}>
                    {output.name === 'body' ? 'loop body' : output.name}
                  </span>
                </div>
              );
            })}
            ...
            {outputs.map((output) => {
              ...
              return (
                <div key={`out-${output.name}`} className="relative flex items-center justify-end gap-1.5 h-4">
                  <span className={cn('text-[10px] font-medium transition-colors', textColorClass)}>
                    {output.name === 'body' ? 'loop body' : output.name}
                  </span>
                  <Handle
                    type="source"
